...
<head>
  ...
  <script type="text/javascript">
  	...
  	function goUpdate(){
  		...
  		console.log(document.frm.memPassword.value);
  		console.log(document.frm.memName.value);
  		console.log(document.frm.memAge.value);
  		console.log(document.frm.memGender.value);
  		console.log(document.frm.memEmail.value); */
    	...
    	<form action="${contextPath }/memUpdate.do" name="frm" method="post">
    		...
    		<table class="table table-bordered" style="text-align: center; border: 1px solid #dddddd;">
    			...
    			<tr>
	    			...
	    			<td colspan="2"><input type="number" id=memAge name="memAge" class="form-control" maxlength="20" placeholder="나이를 입력하세요" value="${mvo.member.memAge }"></td>  
    			</tr>
    			<tr>
	    				...
	    				<div class="form-group" style="text-align: center; margin: 0 auto;">
	    					<div class="btn-group" data-toggle="buttons">
	    						<label class="btn btn-primary <c:if test="${mvo.member.memGender eq '남자' }">active</c:if>">
	    							<input type="radio" id=memGender name="memGender" autocomplete="off" value="남자" <c:if test="${mvo.member.memGender eq '남자' }">checked</c:if>>남자
	    						</label>
	    						<label class="btn btn-primary <c:if test="${mvo.member.memGender eq '여자' }">active</c:if>">
	    							<input type="radio" id=memGender name="memGender" autocomplete="off" value="여자" <c:if test="${mvo.member.memGender eq '여자' }">checked</c:if>>여자
	    						</label>
	    					</div>
	    				</div>
	    			</td>  
    			...
    			<tr>
	    			...
	    			<td colspan="2"><input type="text" id=memEmail name="memEmail" class="form-control" maxlength="20" placeholder="이메일을 입력하세요" value="${mvo.member.memEmail }"></td>  
    			</tr>
    			<!-- 선택한 권한 출력하기 --><!-- 반복문 안에 주석 들어가니 출력에 문제 생김 -->
    			<tr>
    			...
    			<tr>
    				<td colspan="3" style="text-align: left;">
    					<sapn id="passMessage" style="color: red"></sapn><input type="button" class="btn btn-primary btn-sm pull-right" value="수정" onclick="goUpdate()">
    				</td>
    			</tr>
    		</table>
